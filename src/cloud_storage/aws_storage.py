...
class SimpleStorageService:
    ...
    def create_folder(self,folder_name:str,bucket_name:str)->None:
        logging.info("Entered the create_folder method of SimpleStorageSErvice cleint")
        try:
    ...
    def upload_file(self,from_filename:str,to_filename:str,bucket_name:str,remove:bool=True):
        try:
            logging.info(f"Uploading {from_filename} tp {to_filename} in {bucket_name}")
            self.s3_resource.meta.client.upload_file(from_filename,bucket_name,to_filename)
            logging.info(f"Uploaded {from_filename} to {to_filename} in {bucket_name}")

            if remove:
                os.remove(from_filename)
                logging.info(f"Removed local_file {from_filename} after upload")
            logging.info("Exited the upload_file method of SimpleStoreageServie class")
    ...
    def get_df_from_object(self,object_:object)->DataFrame:
        logging.info("Entered the get_df_from_object metod of SimpleStorageSErvice class")
        try:
            content = self.read_object(object_,make_readable=True)
            df=read_csv(content,na_values='na')
            logging.info("Exited the get_df_from_object method of SimepleStorageService class")
            return df
        except Exception as e:
            raise MyException(e,sys) from e
